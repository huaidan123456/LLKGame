...
#include <stdlib.h>
// 幻数定义
const int ALMD5Encode::kA = 0x67452301;
const int ALMD5Encode::kB = 0xefcdab89;
const int ALMD5Encode::kC = 0x98badcfe;
const int ALMD5Encode::kD = 0x10325476;

const unsigned long long ALMD5Encode::k_ti_num_integer = 4294967296;

// function: CycleMoveLeft
// @param src_num:要左移的数
// @param bit_num_to_move:要移动的bit位数
// @return  循环左移后的结果数
UInt32 ALMD5Encode::CycleMoveLeft(UInt32 src_num, int bit_num_to_move) {
...
UInt32 ALMD5Encode::FillData(const char *in_data_ptr, int data_byte_len, char** out_data_ptr) {
...
void ALMD5Encode::RoundF(char *data_BIT_OF_GROUP_ptr, ParamDynamic & param) {
...
void ALMD5Encode::RoundG(char *data_BIT_OF_GROUP_ptr, ParamDynamic & param) {
    UInt32 *M = reinterpret_cast<UInt32*>(data_BIT_OF_GROUP_ptr);
    int s[] = { 5, 9, 14, 20 };
    for (int i = 0; i < 16; ++i) {
...
void ALMD5Encode::RoundH(char *data_BIT_OF_GROUP_ptr, ParamDynamic & param) {
    UInt32 *M = reinterpret_cast<UInt32*>(data_BIT_OF_GROUP_ptr);
    int s[] = { 4, 11, 16, 23 };
    for (int i = 0; i < 16; ++i) {
...
void ALMD5Encode::RoundI(char *data_BIT_OF_GROUP_ptr, ParamDynamic & param) {
    UInt32 *M = reinterpret_cast<UInt32*>(data_BIT_OF_GROUP_ptr);
    int s[] = { 6, 10, 15, 21 };
    for (int i = 0; i < 16; ++i) {
...
void ALMD5Encode::RotationCalculate(char *data_512_ptr, ParamDynamic & param) {
...
// 转换成十六进制字符串输出
std::string ALMD5Encode::GetHexStr(unsigned int num_str) {
    std::string hexstr = "";
...
// return :加密后的MD5值
std::string ALMD5Encode::Encode(std::string src_info) {
    ParamDynamic param;
    param.ua_ = kA;
    param.ub_ = kB;
    param.uc_ = kC;
...
std::string ALMD5Encode::MD5Encode(std::string src_info)
{
    ALMD5Encode encode = ALMD5Encode();
    return encode.Encode(src_info);
}
